...
class _SignUpPageState extends State<SignUpPage> {
 ...
 final userResponse = await _openapi.getAccountResourceApi().registerAccount(managedUserVM: managedUserVM);
  debugPrint('#############################user');
  ...
  final jwtResponse = await _openapi.getAuthenticateControllerApi().authorize(loginVM: loginVM);

  if(jwtResponse.statusCode ==200){

    Configuration.secureStorage.write(key: 'jwt', value: jwtResponse.data?.idToken);
    String? jwt = jwtResponse.data?.idToken;
   UserDTOBuilder userDTOBuilder = UserDTOBuilder();
  // Extract relevant data and assign it to the builder
  userDTOBuilder
    ..login = _loginController.text;
    ...
    UserDTO userDTO = userDTOBuilder.build();
      ...
      final appUserResponse = await _openapi.getAppUserResourceApi().createAppUser(appUserDTO: appUserDTO,headers: {
                              'Authorization': 'Bearer $jwt'
                            });
                            debugPrint('#############################appuser');
      debugPrint('${appUserResponse.statusCode}');
      if (appUserResponse.statusCode == 201) {
          Configuration.secureStorage.write(key: 'appuserId',value : appUserResponse.data?.id.toString());
          Configuration.secureStorage.write(key: 'appUsernumber',value: appUserResponse.data?.phoneNumber.toString());
...
Widget build(BuildContext context) {
  return MaterialApp( // Add MaterialApp or WidgetsApp here
    home :Scaffold(
      appBar: AppBar(
        title: Text('Sign Up'),
      ),
