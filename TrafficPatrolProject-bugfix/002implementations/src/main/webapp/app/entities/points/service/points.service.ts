...
type RestOf<T extends IPoints | NewPoints> = Omit<T, 'uploadDateTime'> & {
...
export class PointsService {
  ...
  protected convertDateFromClient<T extends IPoints | NewPoints | PartialUpdatePoints>(points: T): RestOf<T> {
    return {
      ...points,
      uploadDateTime: points.uploadDateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPoints: RestPoints): IPoints {
    return {
      ...restPoints,
      uploadDateTime: restPoints.uploadDateTime ? dayjs(restPoints.uploadDateTime) : undefined,
    };
  }
